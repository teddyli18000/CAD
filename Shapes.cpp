...
const int kTextMinPixelHeight = 14;//text min
const int kTextMaxPixelHeight = 72;//text max
...
// return: const std::wstring&:文本图元内容引用;
...
// return: const EntityData&:图元参数元数据引用;

//读取线条点集(read-only)
const std::vector<Point2D>& CLine::GetPoints() const { return m_points; }
// return: const std::vector<Point2D>&:图元顶点集合只读引用;
